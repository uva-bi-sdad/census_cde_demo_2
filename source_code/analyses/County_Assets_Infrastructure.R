...
#Remove facilities that are not in Virginia
  temp<-temp[-c(477,1752,2183,2253,3832),-6]
  names(temp)<-c("COUNTY","FAC","LAT","LNG","ZIP","FIP")
  FAC_COUNT<-data.frame(table(temp$COUNTY)); names(FAC_COUNT)<-c("COUNTY","FREQ");rm(temp) 
  dim(FAC_COUNT); View(FAC_COUNT)
...
#Merge with the crosswalk to add the FIPs for each county/city   
    temp<-merge(POP, FAC_COUNT, by="COUNTY", all.x=TRUE, all.y=TRUE); View(temp)
    temp$PER10000<-(temp$FREQ*10000)/temp$POP
  CNTY_ASSETS<-temp; View(CNTY_ASSETS)
    temp<-merge(CNTY_ASSETS, CRWK, by="COUNTY", all.x=TRUE, all.y=TRUE); View(temp)
    CNTY_ASSETS<-temp; dim(CNTY_ASSETS)
    write.csv(CNTY_ASSETS, "~/Documents/Counting People/Data/Derived Variables/CNTY_ASSETS.csv", row.names=FALSE)
...
names(CNTY_ASSETS)<-c("COUNTY","TOTAL","FREQ","PER10000","GEOID" )
CA<-merge(GEOMETRY, CNTY_ASSETS, by="GEOID", all=TRUE); dim(CA)
